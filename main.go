...
func main(){
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		respond(w, "Selamat datang di API Film!! Gunakan /films untuk mengakses data film. (Muhamad Safii - 10122817)", http.StatusOK)
	})
	http.HandleFunc("/films",handleFilms)
	http.HandleFunc("/films/", handleFilmByID)

	fmt.Println("Server berjalan di port 2817") //10122817
	log.Fatal(http.ListenAndServe(":2817", nil))
}
